...
def analysis_mutagen(glist,db):
	...
	for gene in glist:
		...
		for line in gene_info:
			if flag==1:
				if line[5]==' ':
					temp=temp+' '+line.strip()[34:]
				else:
					print temp
					flag=0
			if "FT   MUTAGEN" in line:
				flag=1
				temp=gene+'\t'+line.strip()[17:22]+"\t"+line.strip()[34:]
		if temp=='':
			print gene+"\t"+"no mutation"
		flag=0
def main(argv):
